...
import 'package:flutterconnecct/listscreens/compaignwidget.dart';
import 'package:flutterconnecct/listscreens/createAcompaign.dart';
...
class CompaignListScreens extends StatefulWidget{
  String userid;
  String needid;
  String btntitle;
  CompaignListScreens(String _id,String _needid,String title){
    userid  = _id;
    needid = _needid;
    btntitle = title;
  }
  _CompaignListScreens createState() => new _CompaignListScreens();
}

class _CompaignListScreens extends State<CompaignListScreens>{
  ...
  void initState() {
    ...
    super.initState();
    _getLocation();
    itemShop = CompaignDataModel();
    final FirebaseDatabase database = FirebaseDatabase.instance;
    itemRefShop = database.reference().child('campaigns');
  ...
  _onEntryAddedShop(Event event) {
    setState(() {
      itemsShop.add(CompaignDataModel.fromSnapshot(event.snapshot));
    });
  }

  _onEntryChangedShop(Event event) {
    var old = itemsShop.singleWhere((entry) {
      return entry.organizer_id == event.snapshot.key;
    });
    setState(() {
      itemsShop[itemsShop.indexOf(old)] = CompaignDataModel.fromSnapshot(event.snapshot);
    });
  }

  Position _position;
  List<CompaignDataModel> itemsShop = List();
  CompaignDataModel itemShop;
  ...
  _getLocation() async
  {
    ...
    setState(() {
      txtcontroller.text = first.countryName.toString() + " " + first.addressLine.toString();
    });
  }
                  ...
                  children: [
                    ...
                    Container(
                        ...
                        child: FlatButton(
                          ...
                          onPressed: (){
                            Navigator.of(context).push(MaterialPageRoute(builder: (context) =>  AddACompaign(MyApp.profileDetails.userid.toString())));
                          },
              ...
              Container(
                ...
                child: itemsShop.length==0 ?Center(
                  ...
                  itemBuilder: (context,index){
                    ...
                    return   CompaignWidget(itemsShop[index]);
                    // return   Text("${itemsShop[index].donor_id.toString()}");
                  },
                ),
              ),
