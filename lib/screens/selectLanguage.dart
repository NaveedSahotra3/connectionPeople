...
class SelectLanguage extends StatefulWidget{
  ...
  String id;
  SelectLanguage(String _id){
    id= _id;
  }

  _SelectLanguage createState() => new _SelectLanguage();
...
class _SelectLanguage extends State<SelectLanguage>{
  ...
  void initState() {
    ...
    itemRefShop = database.reference().child('languages');
  ...
  _onEntryAddedShop(Event event) {
    setState(() {
      itemsShop.add(Languages.fromSnapshot(event.snapshot));
    });
  }

  _onEntryChangedShop(Event event) {
    var old = itemsShop.singleWhere((entry) {
      return entry.id_language == event.snapshot.key;
    });
    setState(() {
      itemsShop[itemsShop.indexOf(old)] = Languages.fromSnapshot(event.snapshot);
    });
  }
  ...
  Languages selectedUser;

  setSelectedUser(Languages user) {
    setState(() {
      selectedUser = user;
    });
  }
                      ...
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(7)),
                       ...
                       onPressed: (){
                        ...
                        if(selectedUser.description == ""){
                          ...
                          MyApp.profileDetails.language = selectedUser.description;
                          Navigator.of(context).push(MaterialPageRoute(builder: (context) => SelectAge(widget.id.toString())));
  ...
  void choicehomeAction(String action){
    ...
    print("Homepage");
    Navigator.push(context, MaterialPageRoute(builder: (context) => MainScreen(MyApp.profileDetails.userid.toString())),);
  }
