...
import 'package:flutterconnecct/DataModels/gender.dart';
...
class _SelectGender extends State<SelectGender>{
  ...
  void initState() {
    ...
    itemRefShop = database.reference().child('genders');
  ...
  _onEntryAddedShop(Event event) {
    setState(() {
      itemsShop.add(Gender.fromSnapshot(event.snapshot));
    });
  }

  _onEntryChangedShop(Event event) {
    var old = itemsShop.singleWhere((entry) {
      return entry.id_gender == event.snapshot.key;
    });
    setState(() {
      itemsShop[itemsShop.indexOf(old)] = Gender.fromSnapshot(event.snapshot);
    });
  }
                    ...
                    child: FlatButton(
                      ...
                      onPressed: (){
                        ...
                        if(selectedUser.description == ""){
                          ...
                          MyApp.profileDetails.gender = selectedUser.description;
                          Navigator.of(context).push(MaterialPageRoute(builder: (context) => SelectEducationlevel(widget.id.toString())));
